...
const ProductListItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group rounded-3xl bg-surface shadow-neu">
      <div className="flex">
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4">
          {item.discountedPrice && item.price && item.discountedPrice < item.price && (
            <span className="absolute top-3 left-3 z-10 bg-green-brand text-dark font-semibold text-custom-xs py-1 px-2 rounded-md">
              -{Math.round((1 - item.discountedPrice / item.price) * 100)}%
            </span>
          )}
          <Link href={`/producto/${item.slug || 'default-slug'}`}>
            <Image src={item.imgs?.previews?.[0] || '/images/products/product-1.png'} alt={item.title || "product"} width={250} height={250} />
          </Link>
          ...
          <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
            <button
              ...
              className="flex items-center justify-center w-9 h-9 rounded-xl shadow-neu-sm ease-out duration-200 text-dark bg-surface hover:bg-primary hover:text-white"
            >
            ...
            <Link
              ...
              className="inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-xl bg-primary text-white shadow-neu-sm ease-out duration-200 hover:bg-primary-dark"
            >
              Comprar
            </Link>
          </div>
