...
import ProductItem from "@/tienda/components/Common/ProductItem";
import ProductListItem from "@/tienda/components/Common/ProductListItem";
...
const ShopWithSidebar = ({ products, categoriesList, initialQ = "" }: ShopWithSidebarProps) => {
  ...
  const filteredProducts = products
    ...
    .filter((p) => !searchQuery.trim() || p.title.toLowerCase().includes(searchQuery.toLowerCase().trim()));
  ...
  // Map string categories to the required object shape, calculating the product count
  const categories = categoriesList.map((cat) => ({
    name: cat,
    products: products.filter(p => p.categories?.includes(cat)).length || 0,
    isRefined: false,
  }));
  ...
  return (
    <>
      <Breadcrumb
        ...
        pages={["tienda", "/", "tienda con barra lateral"]}
      />
      <section className="overflow-hidden relative pt-20 pb-20 bg-surface">
          ...
          <div className="flex gap-7.5">
            ...
            <div
              className={`sidebar-content fixed xl:z-1 z-9999 left-0 top-0 xl:translate-x-0 xl:static max-w-[310px] xl:max-w-[270px] w-full ease-out duration-200 ${productSidebar
                ? "translate-x-0 bg-surface p-5 h-screen overflow-y-auto"
                : "-translate-x-full"
                }`}
            >
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex flex-col gap-6">
                  {/* <!-- filter box --> */}
                  <div className="bg-surface shadow-neu rounded-2xl py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Filtros:</p>
                      <button onClick={handleClearFilters} className="text-primary">Limpiar Todo</button>
                    </div>
                  </div>
            ...
            <div className="xl:max-w-[870px] w-full">
              <div className="rounded-2xl bg-surface shadow-neu pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  {/* <!-- top bar left --> */}
                  <div className="flex flex-wrap items-center gap-4">
                    <button
                      ...
                      className="xl:hidden flex items-center justify-center gap-2 px-3 h-9 rounded-xl shadow-neu-sm bg-surface text-dark hover:bg-primary hover:text-white ease-out duration-200"
                    >
                      <svg
                      ...
                      <span className="text-custom-sm font-medium">Filtros</span>
                    </button>
                    <CustomSelect options={options} />

                    <p>
                      Mostrando <span className="text-dark">{filteredProducts.length} de {products.length}</span>{" "}
                      Productos
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5">
                    <button
                      ...
                      className={`${productStyle === "grid"
                        ...
                        : "text-dark bg-surface shadow-neu-sm"
                        } flex items-center justify-center w-10.5 h-9 rounded-xl ease-out duration-200 hover:bg-primary hover:text-white`}
                    >
                      <svg
              ...
              {searchQuery.trim() && (
                <div className="flex items-center justify-between mb-4 px-4 py-2.5 bg-surface rounded-xl shadow-neu-sm">
                  <p className="text-sm text-dark">
                    Resultados para: <span className="font-semibold">&ldquo;{searchQuery}&rdquo;</span>
                    <span className="text-body ml-2">({filteredProducts.length} {filteredProducts.length === 1 ? "producto" : "productos"})</span>
                  </p>
                  <Link href="/tienda" className="text-primary text-sm hover:underline ml-4 shrink-0">
                    × Limpiar
                  </Link>
                </div>
              )}
              ...
              {filteredProducts.length === 0 && searchQuery.trim() ? (
                <div className="py-16 text-center">
                  <p className="text-dark font-medium mb-1">Sin resultados para &ldquo;{searchQuery}&rdquo;</p>
                  <p className="text-body text-sm mb-4">Probá con otro término o explorá todas las categorías.</p>
                  <Link href="/tienda" className="text-primary text-sm hover:underline">
