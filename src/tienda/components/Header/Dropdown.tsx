...
import { Menu } from "@/tienda/types/Menu";
...
const Dropdown = ({ menuItem, stickyMenu }: { menuItem: Menu; stickyMenu: boolean }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-primary text-custom-sm font-medium text-dark flex items-center gap-1.5 capitalize ${
      ...
      >
        {menuItem.submenu?.map((item: Menu, i: number) => (
          <li key={i}>
            <Link
              href={item.path ?? "/"}
              className={`flex text-custom-sm hover:text-primary hover:bg-gray-1 py-[7px] px-4.5 ${
                pathUrl === item.path && "text-primary bg-gray-1"
              } `}
            >
              {item.title}
            </Link>
